...
class PackageManager:
    ...
    def install_requirements(self, requirements_file: str, use_cache: bool = True) -> List[str]:
        ...
        Returns:
            List[str]: List of packages that failed to install
        """
        failed_packages = []
        ...
        try:
            ...
            logger.info(f"Processing {total} requirements from {requirements_file}")
            
            for i, req in enumerate(requirements, 1):
                logger.info(f"[{i}/{total}] Processing {req}")
                if not self.install_package(req, use_cache):
            ...
            logger.error(f"Error processing requirements file {requirements_file}: {e}")
            return [requirements_file]
