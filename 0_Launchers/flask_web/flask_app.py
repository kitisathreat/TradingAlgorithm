...
# --- Start of Definitive Path Fix ---
# Clear and correct sys.path for both local and EC2 deployment.
# This ensures that modules from _2_Orchestrator_And_ML_Python can be found.
project_root = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(project_root))
print(f"[INFO] Added project root to Python path: {project_root}")
# --- End of Definitive Path Fix ---
...
try:
    from rate_limiter import get_stock_data_with_rate_limiting, get_rate_limiter_stats, clear_yfinance_cache, reset_yfinance_rate_limits, EnhancedStockDataFetcher
    RATE_LIMITER_AVAILABLE = True
    print("[INFO] Rate limiter imported successfully")
except ImportError as e:
...
# Initialize enhanced stock data fetcher
if RATE_LIMITER_AVAILABLE:
    stock_data_fetcher = EnhancedStockDataFetcher()
else:
    stock_data_fetcher = None

# Import stock selection utilities
try:
    from _2_Orchestrator_And_ML_Python.stock_selection_utils import StockSelectionManager
...
def get_stock_data(symbol, days=30):
    ...
    #     try:
    #         stock_info, data = stock_data_fetcher.get_stock_data(symbol, days)
    ...
    try:
        ...
        logger.info(f"[TIMING] get_stock_data({symbol}, {days}) exception after {time.time()-t0:.2f} seconds")
        return None, None
...
def get_stock_options():
    ...
    try:
        ...
        try:
            ...
            sp100_paths = [
                # Correct path for our EC2 deployment structure
                Path(__file__).parent.parent / "_2_Orchestrator_And_ML_Python" / "sp100_symbols_with_market_cap.json",
            ...
            logger.info("Searching for S&P100 data file...")
            sp100_file = None
...
def load_stock():
    ...
    try:
        ...
        t1 = time.time()
        logger.info(f"[TIMING] get_stock_data({symbol}, {days}) took {t1-t0:.2f} seconds")
        if stock_info and stock_data is not None:
            ...
            t3 = time.time()
            logger.info(f"[TIMING] calculate_technical_indicators took {t3-t2:.2f} seconds")
            ...
            logger.info(f"[TIMING] /api/load_stock total time: {total_time:.2f} seconds")
            return jsonify({
                'success': True,
            ...
            total_time = time.time() - start_time
            logger.info(f"[TIMING] /api/load_stock total time: {total_time:.2f} seconds (FAILED)")
            return jsonify({
                'success': False,
        ...
        total_time = time.time() - start_time
        logger.info(f"[TIMING] /api/load_stock total time: {total_time:.2f} seconds (EXCEPTION)")
        return jsonify({
            'success': False,
            'error': str(e)
        })
...
@app.route('/api/clear_yfinance_cache', methods=['POST'])
...
@app.route('/api/reset_rate_limits', methods=['POST'])
def reset_rate_limits_endpoint():
    """Reset rate limits"""
    if not RATE_LIMITER_AVAILABLE:
        return jsonify({
            'success': False,
...
@app.route('/api/test_rate_limiting', methods=['POST'])
def test_rate_limiting_endpoint():
    ...
    try:
        ...
        for symbol in symbols:
            try:
                stock_info, data = get_stock_data(symbol, days)
                results[symbol] = {
                    'success': stock_info is not None and data is not None,
                    'data_points': len(data) if data is not None else 0,
                    'synthetic': stock_info.get('synthetic', False) if stock_info else False
                }
            except Exception as e:
                results[symbol] = {
                    'success': False,
